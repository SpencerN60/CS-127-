# TODO: Student writes their name at the top of the file in the comment block below. 
#       (delete this TODO line when completed) (1 pt.)

# <Spencer Ness>             <03/25/23>
# <Assignment 4>
# <Description>
...
def printTitleMaterial():
    ...
    print("By: <Spencer Ness>")
    print("[COM S 127 <section B2>]")
    print()
...
def getColumnInt(board, playerNumber): #this never has to be directly called because its called in "getInputRange"
...
def getInputInRange(board, playerNumber): # This is called in getInputInRange
...
def checkForNextMoveWin(board, playerNumber):
    ...
    for row in board:
        ...
        # use getOpenRow() to check if the column has an open row, and return the row number if there is one
            rowOpen = getOpenRow(board,col)
            if rowOpen != -1:
                placePiece(rowOpen,col,playerNumber)
                ifWinner = checkWinner(playerNumber)
                if ifWinner == True:
                    placePiece(board, rowOpen, col, ".")
...
def checkWinner(board, playerNumber): # Done
